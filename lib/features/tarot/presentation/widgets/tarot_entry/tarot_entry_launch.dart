...
abstract final class TarotEntryLaunch {
  ...
  static Future<bool> start({
    ...
    if (TarotFirstReading.shouldUseFirstSpread(context, ref)) {
      await FirstSessionIntent.consumePendingFirstReading(
    ...
    if (revisit != null) {
      await TarotRevisitIntentStore(ref.read(localStorageProvider)).write(revisit);
    }
    if (!context.mounted) return false;
    ...
    scope?.flow.captureIntention(
      TarotIntention(text: ReadingQuestion.sanitize(question), topic: 'general'),
    );
    if (!context.mounted) return false;
