...
class ReadingVersionService {
  ...
  Future<ReadingVersionAppendResult> tryAppendRevision({
    ...
    // Never invent "Original" from the revision being appended — callers
    // must seedOriginal with the pre-change reading first when no group
    // exists (legacy / prior seed failure).
    if (group == null || group.entries.isEmpty) {
      throw StateError('reading version group missing');
    }
    ...
    return ReadingVersionAppendResult(added: true, group: next);
  }
