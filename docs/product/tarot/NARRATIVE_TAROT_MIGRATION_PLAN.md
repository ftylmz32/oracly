...
| Shadow generation | Where cost allows: produce V2 alongside legacy for corpus, discard from user UI |
| Fixed corpus | Phase 2 pack of questions × spreads × orientations |
| Contract tests | Request/result schema + evidence invariants |
| Old-vs-new comparison | Human + automated anti-generic / anti-certainty |
| Quality gate | Extend `AiOutputQualityTarot` for narrative fields |
| Visual result | Phase 7 — not a precondition for shadow text validation |
| Production switch | Only after Phase 8–10 evidence |
...
**Rationale:** Phase 0 reuse without risky catalogue rewrites; incomplete profiles fall back; testable.
...
**Deterministic:** facts, profiles attach, positions, relationships candidates, recurrence, memory filter, quality checks, settlement.  
**AI:** narrative prose + beat text + optional recurringInsight wording **only if evidence present**.

### D. Where recurrence is calculated

**Locally**, in Narrative Evidence Builder / dedicated `RecurringCardSelector`, **before** AI.  
AI never invents counts.
...
- [ ] Phase 2 corpus green on narrative quality metadata  
- [ ] Recurrence false-positive tests green  
- [ ] R1 privacy + R2/R2.1 fail-closed still green  
- [ ] Legacy reopen tests green  
- [ ] Economy exactly-once tests green  
