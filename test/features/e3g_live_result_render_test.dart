/// E3G — render real Coffee/Palm provider envelopes in result widgets.
library;
...
void main() {
  ...
  testWidgets('E3G coffee live envelope renders without overflow', (tester) async {
    final envelope = jsonDecode(
      File('tool/e3e_private/evidence/coffee_analysis_e3g_call10_response.json')
          .readAsStringSync(),
    ...
    expect(find.textContaining('demlik'), findsWidgets);
  });

  testWidgets('E3G palm structural (E3F envelope; no E3G palm success) envelope renders without overflow', (tester) async {
    final envelope = jsonDecode(
      File('tool/e3e_private/evidence/palm_analysis_e3f_retry_response.json')
          .readAsStringSync(),
    ) as Map<String, dynamic>;
    expect(envelope['success'], isTrue);
    final data = envelope['data'] as Map<String, dynamic>;
    ...
    expect(find.textContaining('Avuç'), findsWidgets);
  });
}