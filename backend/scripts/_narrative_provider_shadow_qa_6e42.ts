/**
 * Phase 6E.4.2 — CLEAN Narrative provider shadow QA after schema compat fix.
 * HARD CAP: 6 real provider calls. NO retry. NO billing. NO persistence.
 * Does NOT overwrite 6E.4 historical artifacts.
...
const CAP = 6;
const QA_RUN_HEAD = '2799e9bf518b2e1db6e9ce338643bffef8c4e632';
const ROOT = resolve(process.cwd(), '..');
const PAYLOADS = resolve(
  ROOT,
  'test/fixtures/tarot_narrative_provider_shadow_payloads_6e42_v2.json',
);
  ...
  for (let i = 0; i < dump.entries.length; i++) {
    ...
    try {
      const result = await service.handle(validated, undefined, {
        identity: 'qa-6e42-shadow',
        parentKey: `qa-6e42:${e.manifestId}`,
      });
