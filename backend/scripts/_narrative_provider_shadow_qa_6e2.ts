/**
 * Phase 6E.2 — CONTROLLED Narrative provider shadow QA.
...
const CAP = 6;
const QA_RUN_HEAD = '3987f7a7851ba24b27ab58b36d3c0f2c3f04025b';
...
function assertHead(): void {
  // Caller must pass --qa-head=<sha>; refuse mismatch.
  const arg = process.argv.find((a) => a.startsWith('--qa-head='));
  const head = arg?.slice('--qa-head='.length) ?? '';
  if (head !== QA_RUN_HEAD) {
    console.error(`HEAD LOCK FAIL: expected ${QA_RUN_HEAD} got ${head || '(missing)'}`);
    process.exit(2);
  }
}
...
async function main() {
  ...
  // --- PRECALL: validate all six BEFORE any provider call ---
  ...
  const observingFetch: OpenAiFetch = async (url, init) => {
    ...
    lastProviderRequestId =
      ...
      null;
    // Clone to read id from body without consuming for transport.
    try {
  ...
  for (let i = 0; i < dump.entries.length; i++) {
    ...
    console.log(`CALL #${callNumber} ${e.manifestId} (remaining=${ledger.remaining})`);
    ...
    try {
      const result = await service.handle(validated, undefined, {
        identity: 'qa-6e2-shadow',
        parentKey: `qa-6e2:${e.manifestId}`,
      });
      const latencyMs = Date.now() - t0;
      // Successful handle returns structured Narrative fields (not {text}).
      if (
        result &&
      ...
      typedFailure = sanitizeFailure(err);
      calls.push({
        manifestId: e.manifestId,
        callNumber,
        languageCode: e.languageCode,
      ...
      // NO retry — continue to next entry only
    }
  }
